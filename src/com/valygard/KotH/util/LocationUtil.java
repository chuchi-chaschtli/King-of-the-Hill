...
public class LocationUtil {
	 ...
	 * <https://github.com/
	 * sk89q/worldedit/blob/master/src/main/java/com/sk89q/worldedit
	 * /EditSession.java> (Line 1249)
	 * 
	 * @param center a location
	 * @param radius a double
	 * @return
	 */
	public static Set<Location> getCircularBoundary(Location center,
			double radius) {
		if (radius <= 0)
	...
	public static Set<Location> getSquareBoundary(Location center, double radius) {
		...
		for (int xn = x - ceilRadius; xn <= x + ceilRadius; xn++) {
			for (int zn = z - ceilRadius; zn <= z + ceilRadius; zn++) {
				boolean border = ((xn == x - ceilRadius || xn == x + ceilRadius) && (zn == z
						- ceilRadius || zn == z + ceilRadius));
				if (border)
