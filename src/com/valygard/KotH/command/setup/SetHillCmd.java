...
public class SetHillCmd implements Command {
	...
	public boolean execute(ArenaManager am, CommandSender sender, String[] args) {
		...
		if (warps == null) {
			warps = am.getConfig()
					.createSection("arenas." + args[0] + ".warps");
			am.saveConfig();
		}
		...
		Location l = new Location(p.getWorld(), x, p.getLocation().getBlockY(),
				z);
		...
		if (args.length == 1) {
			if (s == null) {
				...
				arena.getWarps().getConfigurationSection("hills")
				...
				for (int i = 0; i <= s.getKeys(false).size(); i++) {
					...
					if (s.getString(String.valueOf(i)).equals(
							s.getString(String.valueOf(i + 1)))) {
						Messenger.tell(p,
								"There is already a hill at this location.");
			...
			if (s.getString(String.valueOf(number)) != null) {
				...
				Messenger
						.tell(p, "There is no hill with the specified number.");
				return true;
			}
		}
		am.saveConfig();
