...
public class InfoCmd implements Command {
	...
	public boolean execute(ArenaManager am, CommandSender sender, String[] args) {
		Arena arena;
		arena = (args.length < 1 ? am.getOnlyArena() : am
				.getArenaWithName(args[0]));
		...
		Messenger.tell(sender,
				"Information for " + ChatColor.YELLOW + arena.getName() + ":");

		if (!arena.isRunning()) {
			sender.sendMessage(arena.isEnabled() ? ChatColor.GREEN + "Enabled"
					: ChatColor.RED + "Disabled");
			sender.sendMessage(arena.isRunning() ? ChatColor.RED
					+ "The arena has started." : ChatColor.GREEN
					+ "The arena is not in progress.");
			sender.sendMessage(arena.isReady() ? ChatColor.GREEN + "Ready"
			...
			if (sender instanceof Player) {
				...
				sender.sendMessage(am.getPermittedArenas(p).contains(arena) ? ChatColor.YELLOW
						...
						: ChatColor.RED
								+ "You do not have permission to join this arena.");
			}
			sender.sendMessage(" ");
		}

		if (arena.isRunning()) {
			sender.sendMessage(ChatColor.RED + "Red Team Score: "
					+ arena.getHillTimer().getRedScore());
			sender.sendMessage(ChatColor.BLUE + "Blue Team Score: "
		...
		if (arena.getSettings().getBoolean("arena-stats")) {
			...
			sender.sendMessage(ChatColor.GRAY
					...
					+ (ai.getRating() >= 50.0 ? ChatColor.DARK_GREEN
							: ChatColor.DARK_RED)
					+ (ai.getRating() == 100.0 ? "100" : String.valueOf(
							Math.ceil(ai.getRating())).substring(0, 2)));
